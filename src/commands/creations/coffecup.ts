...
export default class Command extends BaseCommand {
    constructor(client: WAClient, handler: MessageHandler) {
      ...
      command: "coffecup",
			...
			aliases: ["cc","cup"],
			category: "creation",
			usage: `${client.config.prefix}coffecup`,
			baseXp: 50,
        })
    }
    // static count = 0
    run = async (M: ISimplifiedMessage, { joined }: IParsedArgs): Promise<void> => {
        ...
        return void M.reply( await request.buffer(`https://api-xcoders.xyz/api/photooxy/coffecup?text=${chitoge}&apikey=nMLptkNt9s`),
        MessageType.image,
                    undefined,
                    undefined,
                    `✨ Here you go.\n`,
